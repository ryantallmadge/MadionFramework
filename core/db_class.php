...
define('MYSQL_TYPES_NUMERIC', '1 2 3 4 5 8 9 246');
...
define('MYSQL_TYPES_STRING', '253 254 252 247 ');

class db_class {
 
   var $last_error;         // holds the last error. Usually mysql_error()
   var $last_connect_error = true; // holds the last connect error. Usually mysql_error()
   ...
   function db_class() {
	  ...
	  // Constant parama can be found in core/configs/main_config/db.php
   ...
   function connect($host='', $user='', $pw='', $db='') {
	  ...
	  if ($this->db_link->connect_errno) {
		  $this->last_error = "DB Connect failed: " . $this->db_link->connect_error;
   ...
   function dump_query($sql) {
      ...
      $i = 0;
      while ($row = $this->db_link->fetch_assoc($r)) {
         if ($i == 0) {
   ...
   function delete($sql) {
      ...
      // Performs an Delete SQL query and returns the result pointer or false
      // if there is an error.
      ...
      // Returns false if there is an error.
      if(empty($condition)){
         $this->last_error = "You must pass a codition to the update_array() function.";
         return false;
      }
